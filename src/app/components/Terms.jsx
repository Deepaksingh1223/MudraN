...
export default function Terms() {
  ...
  return (
    <section className="w-full min-w-[360px] relative overflow-hidden">
      <div className="mx-auto max-w-4xl px-4 py-8">
        <div className="terms-wrap font-inter text-sm leading-relaxed text-[#1a1a2e]">
          <div className="terms-alert my-6 rounded-lg border border-[#fcd34d] border-l-4 border-l-[#f59e0b] bg-[#fffbeb] px-[18px] py-3.5 text-[13.5px] leading-relaxed text-[#92400e] [&_strong]:text-[#78350f]">
          ...
          {sections.map((sec) => (
            <div
              key={sec.id}
              className={`acc-item mb-2 overflow-hidden rounded-[10px] border bg-white transition-[border-color,box-shadow] duration-150 ${
                openSection === sec.id
                  ? "border-[#6366f1] shadow-[0_0_0_3px_rgba(99,102,241,0.08)]"
                  : "border-[#e5e7eb]"
              }`}
            >
              <button
                type="button"
                className="acc-btn flex w-full cursor-pointer items-center gap-3 border-0 bg-transparent px-[18px] py-[15px] text-left hover:bg-[#fafafa]"
                onClick={() => toggle(sec.id)}
                aria-expanded={openSection === sec.id}
              >
                <span
                  className={`acc-num flex h-[30px] min-w-[30px] shrink-0 items-center justify-center rounded-md border text-[11px] font-bold ${
                    openSection === sec.id
                      ? "border-[#c7d2fe] bg-[#eef2ff] text-[#6366f1]"
                      : "border-[#e5e7eb] bg-[#f3f4f6] text-[#6366f1]"
                  }`}
                >
                  {String(sec.id).padStart(2, "0")}
                </span>
                <span className="acc-label flex-1 text-[13.5px] font-semibold text-[#111827]">
                  {sec.title}
                </span>
                <span
                  className={`acc-icon shrink-0 text-[13px] text-[#9ca3af] transition-transform duration-200 ${
                    openSection === sec.id ? "rotate-180 text-[#6366f1]" : ""
                  }`}
                >
                  ▾
                </span>
              ...
              {openSection === sec.id && (
                <div
                  className="acc-body animate-fade-in px-[18px] pb-5 pl-[60px] text-[13.5px] leading-[1.8] text-[#374151] [&_p]:mb-2.5 [&_p:last-child]:mb-0 [&_ul]:mb-2.5 [&_ul]:pl-5"
